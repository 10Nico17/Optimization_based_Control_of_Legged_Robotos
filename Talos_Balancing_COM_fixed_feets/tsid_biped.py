...
class TsidBiped:
    ...
    def __init__(self, conf, viewer=pin.visualize.MeshcatVisualizer):
        ...
        # define contacts points, matrix (3, 4) 4 contact points, each point is a 3d point
        contact_Point = np.ones((3, 4)) * (-conf.lz)
        
        # Use foot dimension to compute the contact points
        contact_Point[0, :] = [-conf.lxn, -conf.lxn, conf.lxp, conf.lxp]
        contact_Point[1, :] = [-conf.lyn, conf.lyp, -conf.lyn, conf.lyp]

        # Contact6d: postion and orientation, mu friction: coeff, normal force: fMin,fMax 
        ...
        # reference is intial config of the foot
        contactRF.setReference(H_rf_ref)
        
        # add as strict constaint or penalty 
        if conf.w_contact >= 0.0:
            # as penalty cost, make it easier, feasible
